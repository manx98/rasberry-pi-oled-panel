...
i2c_t* i2c_new()
{
	i2c_t* hd = malloc(sizeof(i2c_t));
	hd->fd = -1;
	return hd;
}

void i2c_close(i2c_t *hd)
{
	if (hd && hd->fd > -1)
	{
		close(hd->fd);
...
int spi_open(spi_t* spi, uint8_t bus, unsigned int mode, uint32_t max_speed)
{
	spi->number = bus >> 4;
	spi->channel = bus & 0x0f;
	return wiringPiSPIxSetupMode(spi->number, spi->channel, mode, max_speed);
}
...
{
	wiringPiSPIxClose(spi->number, spi->channel);
}

int spi_transfer(spi_t *spi, const uint8_t *buf, size_t len)
{
	return wiringPiSPIxDataRW(spi->number, spi->channel, buf, len);
}
...
{
	gpio_t *pin = malloc(sizeof(gpio_t));
...
void init_pin(u8x8_t *u8x8, uint8_t pin) {
	user_data_t *user_data = u8x8_GetUserPtr(u8x8);
	if (u8x8->pins[pin] != U8X8_PIN_NONE && user_data->pins[pin] == NULL) {
		user_data->pins[pin] = gpio_new(pin);
		pinMode(pin, OUTPUT);
...
void write_pin(u8x8_t *u8x8, uint8_t pin, uint8_t value) {
	...
	if (u8x8->pins[pin] != U8X8_PIN_NONE) {
		digitalWrite(user_data->pins[pin]->pin_number, value);
	}
}
...
void init_spi(u8x8_t *u8x8) {
	...
	if (spi_handles[user_data->bus] == NULL) {
		...
		int error = spi_open(spi_handles[user_data->bus], user_data->bus,
...
uint8_t u8x8_byte_arm_linux_hw_i2c(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int,
	...
	case U8X8_MSG_BYTE_END_TRANSFER:
		...
		if (i2c_handles[user_data->bus] == NULL)
		{
			...
			i2c_handles[user_data->bus]->fd = wiringPiI2CSetup(addr);
			if (i2c_handles[user_data->bus]->fd < 0)
			{
		...
		wiringPiI2CRawWrite(i2c_handles[user_data->bus]->fd, user_data->buffer, user_data->index);
		break;
...
uint8_t u8x8_byte_arm_linux_hw_spi(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int,
	...
	case U8X8_MSG_BYTE_SEND:
		...
		spi_transfer(spi_handles[user_data->bus], user_data->buffer, user_data->index);
		break;
