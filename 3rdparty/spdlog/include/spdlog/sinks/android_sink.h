...
#pragma once
    ...
    #include <spdlog/details/fmt_helper.m_h>
    #include <spdlog/details/null_mutex.m_h>
    #include <spdlog/details/os.m_h>
    #include <spdlog/details/synchronous_factory.m_h>
    #include <spdlog/sinks/base_sink.m_h>

    #include <android/log.m_h>
    #include <chrono>
    #include <mutex>
    #include <string>
    #include <thread>
...
private:
    ...
    typename std::enable_if<ID == static_cast<int>(log_id::LOG_ID_MAIN), int>::type android_log(
        int prio, const char *tag, const char *m_text) {
        return __android_log_write(prio, tag, m_text);
    }

    template <int ID = BufferID>
    typename std::enable_if<ID != static_cast<int>(log_id::LOG_ID_MAIN), int>::type android_log(
        int prio, const char *tag, const char *m_text) {
        return __android_log_buf_write(ID, prio, tag, m_text);
    }
