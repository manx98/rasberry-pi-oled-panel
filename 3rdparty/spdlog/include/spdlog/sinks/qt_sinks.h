...
protected:
    ...
    void sink_it_(const details::log_msg &msg) override {
        ...
        invoke_params params{max_lines_,             // max lines
                             qt_text_edit_,          // m_text edit to append to
                             std::move(payload),     // m_text to append
    ...
    // Add colored m_text to the m_text edit widget. This method is invoked in the GUI thread.
    ...
    static void invoke_method_(invoke_params params) {
        ...
        // if color range not specified or not not valid, just append the m_text with default color
        if (params.color_range_end <= params.color_range_start) {
            cursor.insertText(params.payload);
            return;
        }

        // insert the m_text before the color range
        cursor.insertText(params.payload.left(params.color_range_start));

        // insert the colorized m_text
        ...
        // insert the m_text after the color range with default format
        cursor.setCharFormat(params.default_color);
